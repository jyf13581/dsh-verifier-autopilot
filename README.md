...
source task
  ...
  -> has-work gate: code/file tasks need exec-class tool calls (meta and read-only tools excluded) or a real diff
...
- Caller-supplied checks run in the candidate cwd through the first shell of the platform chain that starts (`pwsh` where installed, else Windows PowerShell on win32 or `/bin/sh` elsewhere); each result records which shell ran. They are not a command allowlist sandbox, and a timeout does not kill grandchildren. A check is a harness error — marked invalid, never eliminating a candidate — only when the verdict follows from operator inputs alone (review R2 2.2): the shell could not start, the shell cannot parse the command (a parse-only probe in a neutral temp directory, so an unparseable check never runs in the workspace), or the command's leading program does not resolve on this host (`command -v` / `Get-Command`). Output text is never consulted, so a candidate cannot excuse a real failure by printing `not found`, and a deleted check script (`./run-tests.sh`) eliminates.
- Selections conclude with exactly one `outcome`: `ranked_winner` (margin cleared the gate), `objective_only_result` (legacy; unreachable and no longer produced since review R2 2.3 — it required a strict pass-count order among survivors that all passed every check), `single_candidate_fallback` (one survivor, never compared — retained but never called a winner), `insufficient_evidence` (survivors without verifiable work), `abstain` (margin inside the noise band), or `verifier_unavailable` (infrastructure failure with full objective pass). Historical `winnerBasis=verifier` records predate this gate and are retrospectively abstains (only sel-4fcbc905, margin 0.2985, survives the provisional 0.08 gate).
...
- Dedupe (review R2 2.1): the diff fingerprint hashes every changed or untracked path with its content digest (file bytes, symlink target, exec bit, deletion), so equal fingerprints mean byte-identical workspaces. Survivors are grouped per fingerprint and only the lowest index of each group is ranked (the others are `eliminated` with `duplicate-diff:c<k>` and listed in `dedupedCandidates`); if one group remains the record is the `noSearchSpace` fallback. Candidates with empty diffs never dedupe.
- A candidate that ignores `cancel()` after its timeout is abandoned at `candidateTimeoutMs + cancelGraceMs` (default 30 s) and disposed as a loser (review R2 2.5).

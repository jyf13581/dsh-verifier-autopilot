...
> Current status (2026-09-08): the 裁决9.8 evidence-semantics overhaul has landed and is gate-green (191/191 tests). Selections now end in an explicit `outcome` state machine — `ranked_winner | objective_only_result | single_candidate_fallback | insufficient_evidence | abstain | verifier_unavailable` — guarded by a has-work gate (execution-class tool calls or a real worktree diff), a no-search-space dedupe, a **calibrated margin gate** (top-2 margin < 0.03 ⇒ abstain; 0.03 = 2.2× the measured identical-candidate noise ceiling, see below), and harness-error detection that stops broken check commands from eliminating candidates. Candidate models are probed for liveness before planning. A `single_candidate_fallback` is retained but never framed as a chosen best, and every outcome has its own relay contract. Every selection writes an audit pack to `.data/selection-artifacts/<id>.json` before any cleanup. Legacy five-lane citation audits now trust only prompt-visible evidence IDs, and trace compaction uses one shared 16000-char budget so the final answer cannot be truncated away.
>
...
- Caller-supplied checks run through pwsh in the candidate cwd. They are not a command allowlist sandbox, and Windows timeout does not kill grandchildren. A check whose own shell failed to parse the command (harness error) is marked invalid and never eliminates a candidate.
...
- The margin gate is provisional at 0.08 (`selectionMarginThreshold`) until the C0/C1/C2 calibration experiment replaces it; each record carries `margin`, `marginThreshold`, `marginCondition`, and `marginProvisional` so later reviews know exactly which gate produced the verdict.
...
- Every settled selection and every discard writes an audit pack to `.data/selection-artifacts/<selectionId>.json` BEFORE its workspace can be reclaimed: effective config snapshot (immune to later reload drift), per-candidate diff stats and check tails, margin/threshold/condition, outcome, and the source-integration post-audit (`delivered` is granted only when evidence exists; it defaults to `unknown`).
